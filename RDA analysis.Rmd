...
Detailed variables
```{r setup, include=FALSE}
geo<-read.table("clipboard", header=T, sep="\t") ; geo <- geo[-c(75,76), ]
enviro<-read.table("clipboard", header=T, sep="\t") ; enviro <- enviro[-c(75,76), ] ; enviro<-log(enviro+1)
bio<-read.table("clipboard", header=T, sep="\t") ; bio <- bio[-c(75,76), ] ; bio<-log(bio+1)
species<-read.table("clipboard", header=T, sep="\t") ; species <- species[-c(75,76), ]
```
```{r}
#spatial db-MEM variables were obtained using principal coordinates of neighborhood matrix (PCNM).
geo_dist<-vegdist(geo, method="euclidean")
