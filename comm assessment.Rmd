...
ggplot(scrs) +
  geom_point(mapping = aes(x = NMDS1, y = NMDS2, shape = species$zone)) +
...
library(adespatial)
bd<-beta.div(spec, method="sorensen")

```

#fast plot on node degree vs. closness centrality with %OC---to dientify keystone taxa
```{r}
data<-read.table("clipboard",header=T,sep="\t")
ggplot(data, aes(x=zone, y=xval, colour=yval)) + geom_point() +
